...
def main():
    ...
    train_loader = torch.utils.data.DataLoader(train_set, batch_size=250, shuffle=True, num_workers=12)
    val_set = datasets.CIFAR10(root='./data', train=False, download=True, transform=data_transform["val"])
    val_loader = torch.utils.data.DataLoader(val_set, batch_size=250, shuffle=False, num_workers=0)
    val_num = len(val_set)
